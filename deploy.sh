...
docker build -t edwolb/multi-server:latest -t edwolb/multi-client:$SHA -f ./server/Dockerfile ./server
docker build -t edwolb/multi-worker:latest -t edwolb/multi-client:$SHA -f ./worker/Dockerfile ./worker
